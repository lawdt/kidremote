...
public partial class App : Application
{
    ...
    /// переживать сон, поэтому компьютер просыпается заблокированным. Обычный остаток
    /// при этом сохраняется — выданные минуты не сгорают.
    /// </summary>
    ...
    private void OnChatMessage(ChatMessage message)
    {
        ...
        Dispatcher.BeginInvoke(() =>
        {
            ...
            Log.Write($"реплика родителя: экран {(_overlay.IsVisible ? "закрыт" : "свободен")}");
            Alarm.Message();
